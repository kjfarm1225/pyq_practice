import pandas as pd
import streamlit as st

df = pd.DataFrame({
    "month": ["1月", "2月", "3月", "4月", "5月", "6月"],
    "site": ["MELMB", "MELMB", "MELMB", "MELMB", "MELMB", "MELMB"],
    "production": [1000, 1200, 1150, 1300, 1250, 1400],
    "defects": [2, 5, 3, 9, 4, 10],
})

df["ppm"] = (df["defects"] / df["production"] * 1000000).round(1)

st.title("🚓 品質パトロール司令室")
st.write("月別の不良率ppmを確認し、注意が必要な月をパトロールします。")

chart_df = df.set_index("month")[["ppm"]]

st.line_chart(chart_df)

latest_month = df.iloc[-1]["month"]
latest_ppm = df.iloc[-1]["ppm"]

if st.checkbox("🧭 最新月の判定を表示する"):
    st.write("最新月:", latest_month)
    st.write("最新ppm:", latest_ppm)

    if latest_ppm >= 5000:
        st.error("🚨 緊急パトロール：ppmが高いです。すぐに内容確認が必要です。")
    elif latest_ppm >= 3000:
        st.warning("⚠️ 注意パトロール：ppmがやや高めです。傾向確認をおすすめします。")
    else:
        st.success("✅ 通常巡回：大きな問題はなさそうです。")

if st.checkbox("📋 元データを表示する"):
    st.dataframe(df)

if st.checkbox("🚨 ppmが高い月だけ表示する"):
    alert_df = df[df["ppm"] >= 3000]
    st.dataframe(alert_df)

if st.checkbox("🔄 転置したデータを表示する"):
    st.dataframe(df.T)

if st.checkbox("📝 パトロールメモを表示する"):
    st.write("確認ポイント")
    st.write("・ppmが急に上がった月はないか")
    st.write("・不具合数だけでなく、生産数も一緒に見る")
    st.write("・最新月だけで判断せず、数か月の流れを見る")